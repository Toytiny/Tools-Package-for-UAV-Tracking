...
function run_all_trackers_OTB100(save_dir)                                        % 没有输入，则默认保存在当前文件下下的\all_trk_results\
%%  读取视频序列和groundtruth
where_is_your_OTB100_database_folder = 'D:\BaiduNetdiskDownload\OTB100'; % 包含所有数据集图片序列和groundtruth的路径
%OTB100数据集的视频序列和groundtruth是在一起的
%OTB100共有100个数据集，总共59040帧图像，平均序列长度590.4帧
addpath('.\UAV_Basic_Framework\');

%% Read all video names using grouthtruth.txt
dir_output = dir(fullfile(where_is_your_OTB100_database_folder));             % 获取该文件夹下的所有的txt文件
...
if nargin < 1
    save_dir = [main_folder '\all_trk_results\OTB100\'];              % 保存跑完的结果到指定文件夹
end
...
for tracker_count = 1: tracker_num
    ...
    for dataset_count =15:dataset_num
        ...
        seq = load_video_info_OTB100([database_folder '\' video_name]);% 加载序列信息
